...
int data_process(char* i_) {
  ...
  */
  ...
  if(!strcmp(d_opcode,"0100")) {
		...
		printf("--- This is a MVN instruction. \n");
		MVN(Rd, Rn, Operand2, I, S, CC); //
		return 0;
    } 
	else if(!strcmp(d_opcode,"1100")) {
		printf("--- This is a ORR instruction. \n");
...
int branch_process(char* i_) {
  ...
  char imm24[25]; imm24[24] = '\0';
  for (int i = 0; i < 24; i++)
  {
    imm24[i] = i_[i+8];
  }

  int Imm24 = bchar_to_int(imm24);
...
int transfer_process(char* i_) {
  ...
  char imm12[13]; imm12[12] = '\0';
  for (int i = 0; i < 12; i++)
  {
    imm12[i] = i_[i+20];
  }
  int Imm12 = bchar_to_int(imm12);
